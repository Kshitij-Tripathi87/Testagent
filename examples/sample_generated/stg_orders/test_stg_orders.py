"""Auto-generated contract tests for urn:li:dataset:(urn:li:dataPlatform:snowflake,orders.stg_orders,PROD)."""
import pandas as pd
import pytest
...
def test_not_null_3(df: pd.DataFrame):
    assert df['order_ts'].isnull().sum() == 0, 'null values found in order_ts'

def test_unique_4(df: pd.DataFrame):
    assert df['id'].duplicated().sum() == 0, 'duplicate values found in id'

def test_referential_5(df: pd.DataFrame, upstream_df_5: pd.DataFrame):
    # every id in this dataset must exist upstream
    assert set(df['id']).issubset(set(upstream_df_5['id'])), 'orphaned keys not found in urn:li:dataset:(urn:li:dataPlatform:snowflake,orders.raw_orders,PROD)'

def test_freshness_6(df: pd.DataFrame):
